...
class DATASET(object):
    ...
    def get_label(self):
        labels = []
        for index, annot_path in enumerate(self.annotations):
            with open(annot_path, "r") as fp:
                annotations = json.load(fp)
                labels += [elm["label"] for elm in annotations["shapes"]]
        labels = set(labels)
        self.label_map = {elm: cnt for cnt, elm in enumerate(labels, start=0)}
        if len(self.label_map) == 1:
            self.label_map[list(self.label_map.keys())[0]] = 1
